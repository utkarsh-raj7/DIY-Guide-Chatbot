document.addEventListener("DOMContentLoaded", function () {
  ...
  if (voiceMenuBtn) voiceMenuBtn.addEventListener("click", toggleTTS);
  ...
  function createVoiceControls() {
    ...
    // Create the voice menu button that will directly toggle TTS
    const ttsBtn = document.createElement("button");
    ttsBtn.id = "voice-menu-btn";
    ttsBtn.className = "btn btn-circle voice-menu-btn";
    ttsBtn.title = "Toggle Text-to-Speech";
    ttsBtn.innerHTML = '<i class="fas fa-volume-up"></i>';

    // Add click handler to directly toggle TTS
    ttsBtn.addEventListener("click", toggleTTS);

    // Add the button to the page
    audioControls.insertBefore(ttsBtn, audioControls.firstChild);
  ...
  function addVoiceControlStyles() {
    ...
    styleEl.textContent = `
            ...
            /* Removed hover styling to let the external CSS handle it */
        `;
  ...
  function toggleTTS() {
    ...
    if (voiceMenuBtn) {
      voiceMenuBtn.classList.toggle("active", ttsEnabled);
    }
  ...
  function toggleRecording() {
    ...
    if (window.speechSynthesis && window.speechSynthesis.speaking) {
      ...
      // Hide TTS controls if they're visible
  ...
  function startSpeechRecognition() {
    if (recognition) {
      try {
        ...
        showErrorToast(
          "Could not start speech recognition. Falling back to audio recording."
        );
  ...
  function sendAudioMessage() {
    ...
    fetch("/save_audio", {
      ...
      .then((data) => {
        ...
        // Focus the input field but don't auto-send
        if (messageInput) {
          messageInput.focus();
        }
      })
  ...
  function showToast(message, icon, bgColor) {
    ...
    toast.style.top = "20px"; // Changed from bottom to top
    ...
    toast.style.animation = "fadeInDown 0.3s, fadeOut 0.3s 2.7s forwards";
